...
Kobo- and Kindle-compatible Estonian→Russian dictionaries built from the official EKI Estonian–Russian Dictionary, with extra inflection lookup so declined and conjugated forms resolve in books.
...
Kindle (reads the Kobo zip):

```bash
./scripts/build_kindle.sh
./scripts/build_kindle.sh path/to/dicthtml-et-ru.zip
```
...
Kindle build needs the Kobo zip plus [kindling-cli](https://github.com/ciscoriordan/kindling) (the script downloads it).
...
Changes: conversion to Kobo dicthtml and Kindle MOBI, stress-mark reformatting, trimmed examples, and lookup-key expansion from inflected forms.
